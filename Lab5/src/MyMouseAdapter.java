...
public class MyMouseAdapter extends MouseAdapter {
	...
	public void mouseReleased(MouseEvent e) {
		...
		case 1:		//Left mouse button
			...
			if (!myPanel.GameOver && !myPanel.YouWin) {
				Insets myInsets = myFrame.getInsets();
				int x1 = myInsets.left;
				int y1 = myInsets.top;
				e.translatePoint(-x1, -y1);
				...
				if ((myPanel.mouseDownGridX == -1) || (myPanel.mouseDownGridY == -1)) {
					...
					if ((gridX == -1) || (gridY == -1)) {
						...
						if ((myPanel.mouseDownGridX != gridX) || (myPanel.mouseDownGridY != gridY)) {
							...
							if (myPanel.counter[myPanel.mouseDownGridX][myPanel.mouseDownGridY] == -1) {
	...
	private void surroundings(int n, int m, MyPanel myPanel) {
		...
		if (myPanel.counter[n][m] == -1) {
			myPanel.NoBombSquare--;
			System.out.println(myPanel.NoBombSquare);
